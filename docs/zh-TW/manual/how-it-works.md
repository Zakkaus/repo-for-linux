...
> 為確保直接連線有效，具有特定網路拓撲的進階使用者應確認：設定 [核心參數](/zh-TW/manual/user-guide/kernel-parameters) 並**停用** dae 後，將安裝 dae 的裝置設為閘道時，其他裝置仍可正常連網。例如，存取 223.5.5.5 應收到 "UrlPathError" 回應。在安裝 dae 的裝置上執行 tcpdump 時，應可看見來自用戶端裝置的請求封包。
