...
4. 若繫結至 WAN，請確保 `/etc/resolv.conf` 不直接使用本機外部 DNS。例如，可將其設為 `nameserver 119.29.29.29`；封包經由網路介面傳送時，DNS 流量就會被 dae 劫持。多數情況下，重新開機後 dnsmasq 等 DNS 服務會將 `/etc/resolv.conf` 改回原狀，這很難處理。遇到此情況時，建議解除安裝這些服務，或執行 `sudo chattr +i /etc/resolv.conf`。
