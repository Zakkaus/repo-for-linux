...
> **Note**
> If you find your kernel version is `< 5.17`, follow the [**Upgrade Guide**](/manual/user-guide/kernel-upgrade) to upgrade the kernel to the minimum required version.
...
> **Note**: `Armbian` users can follow the [**Upgrade Guide**](/manual/user-guide/kernel-upgrade) to upgrade the kernel to meet the kernel configuration requirement.
...
See [run on alpine](/manual/tutorials/run-on-alpine).

### macOS

We provide a hacky way to run dae on your macOS. See [run on macOS](/manual/tutorials/run-on-macos).
...
dae can run as a daemon (systemd) service. See [run-as-daemon](/manual/user-guide/run-as-daemon)
...
See [Build Guide](/manual/user-guide/build-by-yourself).
...
See [Reload and suspend](/manual/user-guide/reload-and-suspend).
