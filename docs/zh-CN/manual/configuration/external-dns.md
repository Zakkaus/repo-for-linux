...
4. 如果绑定到 WAN，请确保 `/etc/resolv.conf` 不直接使用本地外部 DNS。例如，可将其设为 `nameserver 119.29.29.29`；数据包经由网卡发送时，DNS 流量就会被 dae 劫持。大多数情况下，重启后 dnsmasq 等 DNS 服务会将 `/etc/resolv.conf` 改回原状，这很难处理。如遇此情况，建议卸载这些服务，或执行 `sudo chattr +i /etc/resolv.conf`。
