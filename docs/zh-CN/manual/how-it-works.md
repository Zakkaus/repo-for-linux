...
> 为实现有效直连，具有特定网络拓扑的高级用户应确保：配置[内核参数](/zh-CN/manual/user-guide/kernel-parameters)并**禁用** dae 后，当将装有 dae 的设备设为网关时，其他设备仍可正常访问网络。例如，访问 223.5.5.5 应返回 "UrlPathError" 响应。在装有 dae 的设备上执行 tcpdump 时，应能看到客户端设备发出的请求数据包。
