...
`arm`、`mips`、`mips64`、`mips64le`、`mipsle` 與 `s390x` 架構的建置不支援 `trace` 建置標籤，因此不提供 `dae trace` 命令。請參閱[建置指南](/zh-TW/dae/user-guide/build-by-yourself#trace-support-per-architecture)。
